...
from cribl_python_api_wrapper.licenses import *


def license_testing():
    resp = get_licenses(base_url=base_url, cribl_auth_token=cribl_auth_token)
    print("response: %s" % resp.json())

    resp = get_license_usage_metrics(base_url=base_url, cribl_auth_token=cribl_auth_token)
    print("response: %s" % resp.json())
