...
def get_lookups(base_url, cribl_auth_token, worker_group=None, verify=True):
...
def export_lookup_file(base_url, cribl_auth_token, lookup_filename, save_to_directory=".",
                       worker_group=None, fleet=None, verify=True):
    headers = {"Content-type": "application/json",
               "Authorization": "Bearer " + cribl_auth_token}
    payload = None
    ...
    try:
        ...
        if group is not None:
            return put(base_url + "/m/" + group + "/system/lookups",
                       headers=headers, data=open(lookup_file), verify=verify)
        else:
